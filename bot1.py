...
async def airport(ctx, code):
    ...
    if not row:
        return await ctx.send("❌ Airport not found")
    ...
    runway = airport.get("runway", "Unknown")
    market = airport.get("market", "Unknown")

    hub_cost = airport.get("hub_cost", 0)

    lat = airport.get("lat", "Unknown")
    lon = airport.get("lon", "Unknown")
    ...
    avg_y = int(dem[0] or 0)
    avg_j = int(dem[1] or 0)
    avg_f = int(dem[2] or 0)
    ...
    embed.add_field(
        ...
        value=(
            f"Runway Length: {runway} ft\n"
            f"Market: {market}%\n"
            f"Hub Cost: ${int(hub_cost):,}\n"
            f"Timezone: {timezone}"
        ),
        inline=False
    )
    ...
    embed.add_field(
        ...
        value=(
            ...
            f"Avg Y Demand: {avg_y}\n"
            f"Avg J Demand: {avg_j}\n"
            f"Avg F Demand: {avg_f}"
        ),
        inline=False
    )
